...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    public SettingsUi(ILogger<SettingsUi> logger,
        UiSharedService uiShared, ARPConfigService configService,
        PairManager pairManager,
        ServerConfigurationManager serverConfigurationManager,
        PlayerPerformanceConfigService playerPerformanceConfigService,
        ARPMediator mediator, PerformanceCollectorService performanceCollector,
        FileUploadManager fileTransferManager,
        FileTransferOrchestrator fileTransferOrchestrator,
        FileCacheManager fileCacheManager,
        FileCompactor fileCompactor, ApiController apiController,
        IpcManager ipcManager, CacheMonitor cacheMonitor,
        DalamudUtilService dalamudUtilService, HttpClient httpClient) : base(logger, mediator, "ARP Synchronos Settings", performanceCollector)
    {
    ...
    private void DrawBlockedTransfers()
    {
        ...
        UiSharedService.ColorTextWrapped("Files that you attempted to upload or download that were forbidden to be transferred by their creators will appear here. " +
                             ...
                             "Ask your paired friend to send you the mod in question through other means, acquire the mod yourself or pester the mod creator to allow it to be sent over ARP.",
    ...
    private void DrawFileStorageSettings()
    {
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.Running, "Open ARP Character Data Hub"))
        {
        ...
        UiSharedService.TextWrapped("ARP stores downloaded files from paired people permanently. This is to improve loading performance and requiring less downloads. " +
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted("Monitoring ARP Storage Folder: " + (_cacheMonitor.ARPWatcher?.Path ?? "Not monitoring"));
        if (string.IsNullOrEmpty(_cacheMonitor.ARPWatcher?.Path))
        {
            ImGui.SameLine();
            using var id = ImRaii.PushId("ARPMonitor");
            if (_uiShared.IconTextButton(FontAwesomeIcon.ArrowsToCircle, "Try to reinitialize Monitor"))
            {
                _cacheMonitor.StartARPWatcher(_configService.Current.CacheFolder);
            }
        }
        if (_cacheMonitor.ARPWatcher == null || _cacheMonitor.PenumbraWatcher == null)
        {
            ...
            UiSharedService.AttachToolTip("Attempts to resume monitoring for both Penumbra and ARP Storage. "
        ...
        else
        {
            ...
            UiSharedService.AttachToolTip("Stops the monitoring for both Penumbra and ARP Storage. "
                + "Do not stop the monitoring, unless you plan to move the Penumbra and ARP Storage folders, to ensure correct functionality of ARP." + Environment.NewLine
        ...
        {
            UiSharedService.ColorTextWrapped("Hint: To free up space when using ARP consider enabling the File Compactor", ImGuiColors.DalamudYellow);
        }
        ...
        if (!_fileCompactor.MassCompactRunning)
        {
            ...
            UiSharedService.AttachToolTip("This will run compression on all files in your current ARP Storage." + Environment.NewLine
            ...
            UiSharedService.AttachToolTip("This will run decompression on all files in your current ARP Storage.");
        }
        else
        {
        ...
        ImGui.Separator();
        UiSharedService.TextWrapped("File Storage validation can make sure that all files in your local ARP Storage are valid. " +
        ...
        UiSharedService.AttachToolTip("You normally do not need to do this. THIS IS NOT SOMETHING YOU SHOULD BE DOING TO TRY TO FIX SYNC ISSUES." + Environment.NewLine
            ...
            + "ARPs storage is self-clearing and will not surpass the limit you have set it to." + Environment.NewLine
    ...
    private void DrawGeneral()
    {
        ...
        _uiShared.DrawHelpText("This will add ARP related right click menu entries in the game UI on paired players.");
        ...
        _uiShared.DrawHelpText("This will add ARP connection status and visible pair count in the Server Info Bar.\nYou can further configure this through your Dalamud Settings.");
        ...
        if (ImGui.Checkbox("Show ARP Profiles on Hover", ref showProfiles))
        {
    ...
    private void DrawPerformance()
    {
        ...
        _uiShared.DrawHelpText("Will show a performance indicator when players exceed defined thresholds in ARPs UI." + Environment.NewLine + "Will use warning thresholds.");
        ...
        _uiShared.DrawHelpText("ARP will print a warning in chat once per session of meeting those people. Will not warn on players with preferred permissions.");
        using (ImRaii.Disabled(!warnOnExceedingThresholds && !showPerformanceIndicator))
        {
            ...
            _uiShared.DrawHelpText("ARP will also print warnings and show performance indicator for players where you enabled preferred permissions. If warning in general is disabled, this will not produce any warnings.");
        }
    ...
    private void DrawServerConfiguration()
    {
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            if (ImGui.BeginTabItem("Character Management"))
            {
                ...
                {
                    UiSharedService.ColorTextWrapped("Characters listed here will automatically connect to the selected ARP service with the settings as provided below." +
                    ...
                    foreach (var item in selectedServer.Authentications.ToList())
                    {
                        ...
                        if (ImGui.Checkbox("Automatically login to ARP", ref isAutoLogin))
                        {
                            item.AutoLogin = isAutoLogin;
                            _serverConfigurationManager.Save();
                        }
                        _uiShared.DrawHelpText("When enabled and logging into this character in XIV, ARP will automatically connect to the current service.");
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                if (_dalamudUtilService.IsWine)
                {
                    ...
                    _uiShared.DrawHelpText("On wine, ARP will automatically fall back to ServerSentEvents/LongPolling, even if WebSockets is selected. "
    ...
    private async Task<(bool Success, bool partialSuccess, string Result)> ConvertSecretKeysToUIDs(ServerStorage serverStorage, CancellationToken token)
    {
        ...
        var oauthCheckUri = ARPAuth.GetUIDsBasedOnSecretKeyFullPath(new Uri(baseUri));
    ...
    private void DrawSettingsContent()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("ARP Synchronos Discord"))
        {
